...
const MyCollection = () => {
  ...
  const onDownload = (url: string) => async () => {
    const blob = await fetch("https://cors-anywhere.herokuapp.com/" + url).then(
      (res) => res.blob()
    );
      ...
      {tokenList.map(([url, key]) => {
        ...
        return (
          <Item key={key}>
            ...
            <div>
              <a
                href={`https://viewblock.io/zilliqa/address/0xed54ee4fc27fcafb038c76c010950d72f3bc2ed1?txsType=nft&specific=${key}&network=testnet`}
                target="_blank"
              >
                View on Exploror
              </a>
              <span onClick={onDownload(url)} style={{ cursor: "pointer" }}>
                Download
              </span>
              <a
